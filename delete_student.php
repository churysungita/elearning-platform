...
if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET["student_id"])) {
    ...
    // Perform the student deletion using a prepared statement
    $deleteQuery = "DELETE FROM students WHERE student_id = ?";
    $stmt = $conn->prepare($deleteQuery);
    $stmt->bind_param("i", $studentId);

    if ($stmt->execute()) {
        // Student deleted successfully, redirect to studentlist.php
        header("Location: studentlist.php");
        exit(); // Ensure that the script stops executing after redirection
    } else {
        // Error deleting student, handle the error (e.g., display an error message)
    ...
    function handleDeleteButtonClick(event) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                alert(xhr.responseText); // Display a message from the server
                // You can also update the table to remove the deleted row
            }
        };
